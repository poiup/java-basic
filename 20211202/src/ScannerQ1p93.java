...
public class ScannerQ1p93 {
	public static void main(String[] args) {
		...
		// 스캐너를 이용해서 won 변수에 원하금액을 입력해주세요
		// 원화와 환율을 이용해서
		// 원화 ?월을 환전시켜 ?(화페)입니다. 로 결과물을 출력해주세요
		
		Scanner scan = new Scanner(System.in);
		
		double rate = 15.89;
		System.out.println("루블 환율은" + rate + "입니다");
		System.out.println("환전할 금액을 입력해주세요");
		int won = scan.nextInt();
		System.out.println("원화 " + won + "원을 환전시켜 " );
		// won / rate를 Math.round()로 감싼다음, 10^n승을 곱하고 10.0^n승으로 나눠주면
		// n승에 해당하는 자리수만큼 소수점 자리까지만 나옵니다.
		System.out.println(Math.round(won / rate * 100) / 100.0 + "₽ 입니다.");
